...
seed_line = "- `simulation.seed-variant` — integer, applied on the next explicit reset."
if "scheduler.chunk.size" not in text:
    text = text.replace(
        seed_line,
        seed_line[:-1]
...
text = text.replace(
    "Deterministic metrics count cell work, future chunk transitions/state, and phase progress.",
    "Deterministic metrics count cell work, actual chunk transitions/state, and phase progress.",
)
text = text.replace(
...
trace_link = "- [`docs/TRACE_PROTOCOL.md`](docs/TRACE_PROTOCOL.md) — trace, metrics, provenance, retention, and evidence-adapter contract."
if "docs/CHUNK_SLEEP_WAKE.md" not in text:
    text = text.replace(
        trace_link,
        trace_link
...
if "## SD-006 chunk sleep/wake scheduler" not in text:
...
trace = Path("docs/TRACE_PROTOCOL.md")
...
old_chunk = "The chunk vocabulary is defined before SD-006 so later sleep/wake work can emit facts through the same presentation-facing contract. The current SD-004 teaching runner has no chunk scheduler and therefore does not fabricate chunk events."
text = text.replace(
    old_chunk,
...
if "### Chunk sleep/wake scheduler" not in text:
    block = """### Chunk sleep/wake scheduler
...
"""
    text = text.replace("## Determinism contract\n", block + "## Determinism contract\n", 1)
architecture.write_text(text)

verify = Path("docs/VERIFY.md")
